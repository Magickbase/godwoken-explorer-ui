...
const CollapsableScript: React.FC<{ script: API.Account.ParsedScript; name: string }> = ({ script, name }) => {
  ...
  return (
    <Accordion sx={{ boxShadow: 'none', width: '100%' }} disabled={!script}>
      <AccordionSummary sx={{ p: 0, textTransform: 'capitalize' }} expandIcon={<ExpandMore />}>
        ...
        <Chip label={script?.name || t('unknownScript')} color="info" variant="outlined" size="small" />
      </AccordionSummary>
